...
from models.CPF_H import CPF
from models.MAGNN import MAGNN_nc, MAGNN_nc_mb
from trainer_CPF import Trainer
...
if tea == stu:
    ...
    student_model = CPF(g=g, graph=graph, byte_idx_train=byte_idx_train, labels_one_hot=labels_one_hot, labels_init=labels_init,  \
...
def train_teacher(epoches):
    ...
    for epoch in range(epoches):
        ...
        loss_test, auc, micro_f1, macro_f1, _, _ = teacher.evaluate(epoch, idx_val, idx_test, labels, teacher_target, teacher_input)
        results_auc += auc
        results_micro_f1 += micro_f1
        results_macro_f1 += macro_f1
        acc_v, _ = micro_f1[0]
...
attn = []
result, loss_s, best_att, best_gt = train_student(opt['max_epoch'])
attn = best_att
pre_dis_s = loss_s
...
auc_test = get_result(student_auc)
micro_test = get_result(student_micro)
macro_test = get_result(student_macro)
