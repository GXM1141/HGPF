...
class Trainer(object):
    ...
    def train(self, idx_train, idx_val, idx_test, soft_target, target, inputs):
        ...
        if self.Mtype == 'student':
            ...
            loss = my_loss(logp[idx_no_train], soft_target[idx_no_train]) + 0.3 * F.kl_div(b[idx_no_train], a[idx_no_train], reduction='batchmean')
    ...
    def evaluate(self, epoch, idx_val, idx_test, target, soft_target, inputs):
        ...
        return loss_test, [(auc_val.item(), auc_test.item())], [(val_micro_f1.item(), test_micro_f1.item())], [(val_macro_f1.item(), test_macro_f1.item())], att, logp
