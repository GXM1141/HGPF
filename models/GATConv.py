...
class myGATConv(nn.Module):
    ...
    def forward(self, graph, x, res_attn=None):
        x = self.feat_drop(x)
        h = torch.matmul(x, self.W).view(-1, self.nhead, self.out_features)#-1补齐 (num_nodes, nhead, feat_dim)
        h[torch.isnan(h)] = 0.0
        e = torch.matmul(self.edge_emb, self.W_e).view(-1, self.nhead, self.edge_feats)#（num_etypes, nhead, edge_feats）
        ...
        with graph.local_scope():
            ...
            for i in range(self.nhead):
                graph.srcdata.update({'ft': h[i]})
                graph.edata['a'] = edge_attention[:, i]
