...
class GTN(BaseModel):
    ...
    def __init__(self, num_edge_type, num_channels, in_dim, in_dims, hidden_dim, num_class, num_layers, category, norm = True,
                 identity = True):
        r"""
            Description
            -----------
        ...
        for t in range(len(in_dims)):
            self.adapt_ws.append(nn.Linear(in_dims[t], in_dim))
        self.A = None
        self.h = None
    ...
    def forward(self, hg, f):

        hd = []
        for i in range(len(self.in_dims)):
            hd.append(F.relu(self.dropout(self.adapt_ws[i](f[i]))))#feature transformation
        h = {'P': hd[0], 'A': hd[1], 'S': hd[2]}
        with hg.local_scope():
            hg.ndata['h'] = h
