...
class PLP(nn.Module):
    def __init__(self, g, feat_dims, num_layers, in_dim, emb_dim, num_classes, activation,
        ...
        self.masked = byte_idx_train#nxk的矩阵，其中已知label的train——idx的每一行全是1
        self.masked_labels_one_hot = th.mul(labels_one_hot, self.masked)#train_idx的label，其他行是0
    ...
    def forward(self, features, label_init):
        x = []
        for feat, fc in zip(features, self.fc_list):
            x.append(self.dropout(fc(feat)))
        features = th.cat(x, dim=0)
        h = label_init#初始化概率转化矩阵
        for l in range(self.num_layers):
            h, att, alpha, el, er = self.plp_layer(self.g, features, h)
            h = th.mul(h, ~self.masked) + self.masked_labels_one_hot#带标签的点标签不变，其他点+h
        # output projection
...
class PLPConv(nn.Module):
    def __init__(self,
        ...
        self.ptype = ptype
        self.lr_alpha = nn.Parameter(th.zeros(size=(node_num, 1)))#learn rate for confident score
        self._allow_zero_in_degree = allow_zero_in_degree
        if self.ptype == 'ind':
            self.attn_l = nn.Parameter(th.FloatTensor(size=(1, in_feats)))#size = size(in_feats)
        elif self.ptype == 'tra':
    ...
    def forward(self, graph, feat, soft_label):
        with graph.local_scope():
            ...
            if self.ptype == 'ind':
                feat_src = h_dst = self.feat_drop(feat)#将起始节点和中止节点(每个节点的两重身份)初始化为节点的feature，接下来要计算cofident scores
                el = (feat_src * self.attn_l).sum(dim=-1).unsqueeze(-1)#features过一个线性层，计算confident scores
            ...
            graph.apply_edges(fn.u_add_v('el', 'er', 'e'))#每条边两头的·节点特征相加，用于计算后来的边权重
